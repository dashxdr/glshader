...
float u( float x ) { return 0.35+0.5*sin(x*cos(x)); }
...
void main(void)
{
	...
	float sides = 4.0 - 2.0*sin(time);
	float rings = 8.0 + 4.0*sin(time);
    float w = cos(3.1415927*td8-r);
    float h = 0.5+0.5*sin(time)+0.5*cos(sides*a-w*sides*rings+r*3.0);
    float d = 0.5+0.75;
	d*=pow(h,8.0*r)*(0.75+0.1*w);
