...
// some non-mathematical tweaks added by Kaeltyk
...
void main( void ) {
	...
	float f0 = 0.0; //fract(time*0.005)+1e-5;
	float f = fract(f0*2.);
	float fs = sign(f-.5);
	float fs0 = sign(f0-.5);
	...
	float as = (sin(time*.01)*.99*m);	// there was originally an outer sinh for as and bs but difference is just about 1 percent which doesn't really matter for the camera path
	float ac = sqrt(as*as+1.);
	float bs = (sin(time*.01)*.99*m);
	...
	for (int i = 0; i < 15; i++) {
		...
		pos += pos*kDist*-0.0 + dir*kDist*1.1;	// compute actual distance (as we're in the klein metric we can't simply do length(a-b) - we have to use
		vec4 hpos2 = kleinToHyper(pos);
		float hdistnow = hyperdist(hpos, hpos2);
		hdist += hdistnow;
		cremain *= exp(-.3*hdistnow); //... and simulate fog
		hpos = hpos2;
