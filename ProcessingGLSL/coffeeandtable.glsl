...
// Fixed PC version by randomly fiddling with raymarch fuction
// Included workaround for AMD Radeon on OS X from an unknown benefactor in #1524 - thanks!
//nah, the glass looks screwed up now
#ifdef GL_ES
...
float GetDistanceMug( const in vec3 vPos )
{
	...
	float r1 = 0.65;
	float r2 = 0.10;
	vec2 q = vec2(length(vPos.xy + vec2(1.1, -0.1))-r1,vPos.z);
...
{
	float fTopCoffee = vPos.y - 0.8 + 0.02*sin(length(vPos.xz)*18.0-2.0*time);
...
{             
	...
	vec4 vScreenDist = vec4(-vPos.y, 5.0, vPos.xz) + 0.02*sin(float(floor(atan(vPos.x,vPos.z)))); // omg broken
...
void main( void )
{
	...
	vec3 vCameraPos = OrbitPoint(-mouse.x * 7.0, (mouse.y + 0.1) * PI * 0.25) * 7.0 - vec3(0.0, 0.9, 0.0);
	#ifdef ENABLE_MONTE_CARLO              
